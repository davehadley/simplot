...
class TestSimpleFit(unittest.TestCase):
    ...
    def test_build_simple_model(self):
        ...
        toymc2, cov = SimpleMcBuilder().build(None, toymc1)
    ...
    def test_eval_model(self):
        npe = 10**4
        toymc1 = self._buildtestmc()
        toymc2, cov = SimpleMcBuilder().build(None, toymc1, npe=npe, keep={"z":[-10.0, -5.0, 0.0, 1.0, 5.0, 10.0]})
...
class TestSimpleFitWithOscillation(unittest.TestCase):
    ...
    def test_build_simple_model_with_osc(self):
        toymc1 = self._buildtestmc()
        toymc2, cov = SimpleMcWithOscillationBuilder().build(None, toymc1, toymc1.ratevector)
        #generate a single event
        toymc2()
        return
    ...
    def test_eval_model(self):
        npe = 10**3
        toymc1 = self._buildtestmc()
        toymc2, cov = SimpleMcWithOscillationBuilder().build(None, toymc1, toymc1.ratevector, npe=npe)
        stat1 = [Mean(), StandardDeviation()]
        stat2 = [Mean(), StandardDeviation()]
        calculate_statistics_from_toymc(toymc1, stat1, npe=npe)
        calculate_statistics_from_toymc(toymc2, stat2, npe=npe)
...
class TestOscillationCalculation(unittest.TestCase):
    ...
    def build1flavmodel(self, iflav, jflav, mode):
        ...
        model = BinnedSampleWithOscillation("flatmodelwithoscillation", binning, observables, gen(10**4), enuaxis="trueenu", flavaxis="nupdg", 
                                            distance=295.0, systematics=systematics, probabilitycalc=None, oscparmode=mode)
        oscgen = OscillationParametersPrior(seed=1225, oscparmode=mode).generator
        systgen = GaussianGenerator(["x", "y", "z"], [0.0, 0.0, 0.0], [0.1, 0.1, 0.1], seed=1226)
        #toymc = ToyMC(model, GeneratorList(oscgen))
        toymc = ToyMC(model, GeneratorList(oscgen, systgen))
        return toymc
    ...
    def test_osccalc(self):
        pars = PdgNeutrinoOscillationParameters()
        for mode in [OscParMode.SINSQTHETA, OscParMode.SINSQ2THETA]:
            flav_other = {0:1, 1:0, 2:3, 3:2}
            for iflav in xrange(4):
                for jflav in [iflav, flav_other[iflav]]:
                    toymc1 = self.build1flavmodel(iflav, jflav, mode=mode)
                    toymc2, cov = SimpleMcWithOscillationBuilder().build(None, toymc1, toymc1.ratevector, oscparmode=mode)
                    oscpars = [pars.value(p) for p in pars.ALL_PARS]
                    self.check_osccalc(toymc1, iflav, jflav, oscpars)
                    self.check_osccalc(toymc2, iflav, jflav, oscpars)
        return

    def check_osccalc(self, toymc, iflav, jflav, oscpars):
